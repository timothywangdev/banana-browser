...
Always use `banana-browser` directly -- never `npx banana-browser`. The direct binary uses the fast Rust client. `npx` routes through Node.js and is significantly slower.
...
```bash
banana-browser --session {SESSION} open {TARGET_URL}
banana-browser --session {SESSION} wait --load networkidle
```
...
```bash
banana-browser --session {SESSION} snapshot -i
# Identify login form refs, fill credentials
banana-browser --session {SESSION} fill @e1 "{EMAIL}"
banana-browser --session {SESSION} fill @e2 "{PASSWORD}"
banana-browser --session {SESSION} click @e3
banana-browser --session {SESSION} wait --load networkidle
```
...
```bash
banana-browser --session {SESSION} state save {OUTPUT_DIR}/auth-state.json
```
...
```bash
banana-browser --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/initial.png
banana-browser --session {SESSION} snapshot -i
```
...
```bash
banana-browser --session {SESSION} snapshot -i
banana-browser --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/{page-name}.png
banana-browser --session {SESSION} errors
banana-browser --session {SESSION} console
```
...
```bash
banana-browser --session {SESSION} record start {OUTPUT_DIR}/videos/issue-{NNN}-repro.webm
```
...
```bash
banana-browser --session {SESSION} screenshot {OUTPUT_DIR}/screenshots/issue-{NNN}-step-1.png
sleep 1
# Perform action (click, fill, etc.)
sleep 1
banana-browser --session {SESSION} screenshot {OUTPUT_DIR}/screenshots/issue-{NNN}-step-2.png
sleep 1
...
```bash
sleep 2
banana-browser --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/issue-{NNN}-result.png
```

4. **Stop the video:**

```bash
banana-browser --session {SESSION} record stop
```
...
```bash
banana-browser --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/issue-{NNN}.png
```
...
```bash
banana-browser --session {SESSION} close
```
...
- **Be efficient with commands.** Batch multiple `banana-browser` commands in a single shell call when they are independent (e.g., `banana-browser ... screenshot ... && banana-browser ... console`). Use `banana-browser --session {SESSION} scroll down 300` for scrolling -- do not use `key` or `evaluate` to scroll.
